define(['b2', 'noise', 'stats'], function(b2, ClassicalNoise, Stats) {
    //model of the game
    //
    ...
    function Game(width, height, gridSize) {
        ...
        var world = this.world = new b2.World(
    ...
    Game.prototype.step = function(keysPressed, mouse) {
        ...
        var POWER = .3;
        var loc = this.you.GetWorldPoint(new b2.Vec2(0, -.5));
        if('w' in keysPressed) {
            this.you.ApplyImpulse(new b2.Vec2(0, -POWER), loc);
        }
        if('a' in keysPressed) {
            this.you.ApplyImpulse(new b2.Vec2(-POWER, 0), loc);
        }
        if('s' in keysPressed) {
            this.you.ApplyImpulse(new b2.Vec2(0, POWER), loc);
        }
