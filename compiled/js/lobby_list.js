(function() {
  require(['jquery', 'underscore'], function($, _) {
    ...
    gamelistTemplate = _.template("    <h2> <%= _.size(lobbies) %> Currently Open Lobbies</h2>    <h3><a href='/new'>Create new game</a></h3>    <% _.each(lobbies, function(lobby, lobbyId) { %>      <div class='lobby'>        <a href=\" <%= lobbyId %> \"> <%= lobbyId %> </a>        <div><%= _.map(lobby.players, function(player) { return player.name; }).join(', ') %></div>        <div><%= lobby.properties.scoreLimit %> to win</div>        <div><%= lobby.properties.mapWidth %> x <%= lobby.properties.mapHeight %></div>        <div>Seed: <%= lobby.properties.randomSeed %> </div>      </div>    <% }); %>    ");
